                        ...
                        @if (count($users) > 0)
                            @foreach ($users as $user)
                                <tr data-entry-id="{{ $user->id }}">
                                    ...
                                    <td>
                                        <a href="{{ route('students.show',[$user->id]) }}" class="btn btn-sm btn-success"><i class="fas fa-eye"></i> View</a>
                                        <a href="{{ route('students.edit',[$user->id]) }}" class="btn btn-sm btn-info"><i class="fas fa-edit"></i> Edit</a>
                                        {!! Form::open(array(
                                            'style' => 'display: inline-block;',
                                            'method' => 'DELETE',
                                            'onsubmit' => "return confirm('".trans("Are you sure?")."');",
                                            'route' => ['students.destroy', $user->id])) !!}
...
@section('javascript')
    <script>
        window.route_mass_crud_entries_destroy = '{{ route('students.mass_destroy') }}';
    </script>
@endsection